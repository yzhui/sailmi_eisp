...
namespace Eisp.DAL
{
    public class EispProClassDAL
    {
        ...
        {
            string sql = "delete from T_ProClass where ID=?";
        ...
        {
            string sql = "select * from T_ProClass where F_ParentID = ? order by F_Sort desc";
            OleDbParameter[] parameter = new OleDbParameter[1];
            ...
            while (dr.Read())
            {
                ...
                proclass.F_ParentID = Convert.ToInt32(dr["F_ParentID"]);
                proclass.F_Sort = Convert.ToInt32(dr["F_Sort"]);
                proclass.F_Lang = Convert.ToInt32(dr["F_Lang"]);
                list.Add(proclass);
        ...
        public static int UPSort(int id,int type)
        {
            ...
            if(type==0)
            {
                sql = "update T_ProClass set F_Sort=F_Sort+1 where ID=?";
            }
            else
            {
                sql = "update T_ProClass set F_Sort=F_Sort-1 where ID=?";
        ...
        {
            string sql = "select count(*) from T_ProClass where F_ParentID=?";
